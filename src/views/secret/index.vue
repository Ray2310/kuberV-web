  ...
  <div class="app-container">
    <div class="data-operation">
      ...
      <div class="data-operation-item">
        <el-button type="primary" @click="redirectTo('/volume/secret-edit')">新增</el-button>
      </div>
    </div>
    <el-table :data="list">
      ...
      <el-table-column label="name" width="300" prop="name">
      </el-table-column>
      <el-table-column label="data" width="100" prop="dataNum"></el-table-column>
      <el-table-column label="type" width="300" prop="type"></el-table-column>
  ...
  methods: {
    doDel(name) {
      ...
      this.$confirm('此操作将永久删除该Secret, 是否继续?', '提示', {
        ...
        this.$store.dispatch("secret/deleteSecret", params).then(res => {
    ...
    getNodeItemOrList() {
      ...
      this.$store.dispatch("secret/getSecretItemOrList", params).then(res => {
        this.list = res.data
      })
    },
    filterNode() {
    ...
    goEdit(name) {
      this.redirectTo("/volume/secret-edit?name=" + name)
    },
    redirectTo(url) {
      this.$router.push(
        {
